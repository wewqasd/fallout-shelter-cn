...
# 输出 {key: [6 语言值]}，即 data/new_version_terms_full.json（EN 源，基准参照）
```

- `data/steam_cn_full.json`：**权威翻译表** {key: 中文}（14064 键），
  `patch_core` 写回时使用；`.tsv` 为同内容编辑用表格（两者必须保持同步）。
- `data/new_version_terms_full.json`：I2 全量 dump（EN+5 语言，索引 0 = EN），
...
- **版本情报**：包内 EN 与基准 `new_version_terms_full.json` 有差异的 term 数 → `en_changed`
